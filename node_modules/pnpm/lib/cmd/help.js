...
function getHelpText(command) {
    switch (getCommandFullName_1.default(command)) {
        ...
        Options:
          ...
          --no-lockfile                      don't read or generate a \`${constants_1.WANTED_LOCKFILE}\` file
          --lockfile-only                    dependencies are not downloaded only \`${constants_1.WANTED_LOCKFILE}\` is updated
          --frozen-lockfile                  don't generate a lockfile and fail if an update is needed
          --prefer-frozen-lockfile           if the available \`${constants_1.WANTED_LOCKFILE}\` satisfies the \`package.json\`
                                             then perform a headless installation.
          --lockfile-directory <path>        the directory in which the ${constants_1.WANTED_LOCKFILE} of the package will be created.
      ...
      `;
        ...
        Generates ${constants_1.WANTED_LOCKFILE} from an npm package-lock.json (or npm-shrinkwrap.json) file.
      `;
        case 'uninstall':
            return common_tags_1.stripIndent `
