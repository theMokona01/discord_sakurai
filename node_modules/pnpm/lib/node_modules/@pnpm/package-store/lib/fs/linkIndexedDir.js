...
const hardLinkingLogger = logger_1.default('_hardlink-already-exists');
async function linkIndexedDir(existingDir, newDir, filenames) {
...
async function tryLinkIndexedDir(existingDir, newDir, filenames) {
    const alldirs = new Set();
    filenames
        .forEach((f) => {
        alldirs.add(path.join(newDir, path.dirname(f)));
    });
    await Promise.all(Array.from(alldirs).sort((d1, d2) => d1.length - d2.length).map((dir) => mkdirp(dir)));
    let allLinked = true;
    ...
    if (!allLinked) {
        logger_1.storeLogger.warn(`Not all files from "${existingDir}" were linked to "${newDir}". ` +
